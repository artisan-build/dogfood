...
test('config file returns an array', function () {
    $config = include __DIR__.'/../../config/opencode-sdk.php';

    expect($config)->toBeArray();
});

test('config has expected structure with all required keys', function () {
...
test('config base_url has correct default value', function () {
    $config = config('opencode-sdk');

    expect($config['base_url'])->toBe('http://localhost:3333');
});

test('config timeout has correct default value', function () {
    $config = config('opencode-sdk');

    expect($config['timeout'])->toBe(30);
});

test('config auth type has correct default value', function () {
    $config = config('opencode-sdk');

    expect($config['auth']['type'])->toBeNull();
});

test('config auth token has correct default value', function () {
    $config = config('opencode-sdk');

    expect($config['auth']['token'])->toBeNull();
});

test('config retry times has correct default value', function () {
    $config = config('opencode-sdk');

    expect($config['retry']['times'])->toBe(3);
});

test('config retry sleep has correct default value', function () {
    $config = config('opencode-sdk');

    expect($config['retry']['sleep'])->toBe(100);
});

test('config base_url can be overridden by environment variable', function () {
...
test('config timeout can be overridden by environment variable', function () {
...
test('config auth type can be overridden', function () {
...
test('config auth token can be overridden', function () {
...
test('config retry times can be overridden', function () {
...
test('config retry sleep can be overridden', function () {
