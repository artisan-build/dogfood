<p align="center"><img src="https://github.com/artisan-build/forge-sdk/raw/HEAD/art/forge-sdk.png" width="75%" alt="Artisan Build Forge SDK Logo"></p>
...
```bash
composer require artisan-build/forge-sdk
```
...
```bash
php artisan vendor:publish --tag="forge-sdk-config"
```

This creates `config/forge-sdk.php` where you can configure API settings, logging, retry behavior, and defaults.
...
$forge->authenticate(config('forge-sdk.api_token'));
...
```php
use ArtisanBuild\ForgeSdk\Enums\CertificateType;
...
```php
use ArtisanBuild\ForgeSdk\Enums\LogType;
...
```php
use ArtisanBuild\ForgeSdk\Enums\MonitorType;
...
```php
use ArtisanBuild\ForgeSdk\Exceptions\{
    ValidationException,
    AuthenticationException,
    RateLimitException,
    ApiException,
...
try {
    ...
    Log::error('Forge SDK error', ['message' => $e->getMessage()]);
}
```
