@php use ArtisanBuild\Hallway\Channels\Models\Channel;use ArtisanBuild\Hallway\Members\Models\Member;use ArtisanBuild\HallwayFlux\Livewire\Layout\ChannelsComponent; @endphp
    <!DOCTYPE html>
...
<flux:sidebar sticky stashable class="bg-zinc-50 dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-700">
    ...
    <flux:navlist variant="outline">
        ...
        <flux:navlist.item icon="calendar" wire:navigate
                           href="{{ route(config('hallway-flux.route-name-prefix') .'calendar') }}">Calendar
        </flux:navlist.item>
            ...
            <flux:menu>
                ...
                <form method="post" action="{{ route('logout') }}">
                    ...
                    <flux:menu.item icon="arrow-right-start-on-rectangle"
                                    type="submit">{{ __('Log Out') }}</flux:menu.item>
                </form>
            </flux:menu>
    ...
    @endauth
    <flux:navlist variant="outline">
        <flux:navlist.item icon="building-storefront" target="_blank" href="https://hallway.fm">Powered by Hallway.fm
        </flux:navlist.item>
    </flux:navlist>
</flux:sidebar>
...
<flux:header class="!block bg-white lg:bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-700">
    ...
    @else
        <flux:navbar class="{{ \Illuminate\Support\Facades\Auth::guest() ? 'justify-end' : '' }}" scrollable>
            @auth
            <flux:navbar.item wire:navigate href="{{ route(config('hallway-flux.route-name-prefix') .'lobby') }}">Lobby
            </flux:navbar.item>
            <flux:navbar.item badge="{{ Member::count() }}" wire:navigate
                              href="{{ route(config('hallway-flux.route-name-prefix') . 'members') }}">Members
            </flux:navbar.item>
            <flux:navbar.item wire:navigate href="{{ route(config('hallway-flux.route-name-prefix') .'mentions') }}">
                Mentions
            </flux:navbar.item>
            <flux:navbar.item wire:navigate href="{{ route(config('hallway-flux.route-name-prefix') .'bookmarks') }}">
                Bookmarks
            </flux:navbar.item>
            @else
                <flux:navbar.item wire:navigate href="{{ route('register') }}">Register</flux:navbar.item>
                <flux:navbar.item wire:navigate href="{{ route('register') }}">Log In</flux:navbar.item>
            @endif
        </flux:navbar>
    @endif
</flux:header>
